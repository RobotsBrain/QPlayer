...
SdlVideoFrame::SdlVideoFrame(SDL_Texture* overlay)
: IFrame(nullptr)
, overlay_(overlay)
{
}
...
SDL_Texture* SdlBlitter::sdl_overlay(SdlVideoFrame* frame)
{
...
//						src->frame->linesize, 0, src->frame->height,
...
, overlay_(nullptr)
{
    ...
    char variable[64];
    snprintf(variable, sizeof(variable), "SDL_WINDOWID=0x%llx", winId());
    putenv(variable);
...
//    screen_ = SDL_SetVideoMode(640, 480, 24, 0);
    screen_ = SDL_CreateWindow("QPlayer", SDL_WINDOWPOS_UNDEFINED, SDL_WINDOWPOS_UNDEFINED,
...
//    overlay_ = SDL_CreateYUVOverlay(width, height, SDL_YV12_OVERLAY, screen_);

    sdlRenderer_ = SDL_CreateRenderer(screen_, -1, 0);
    overlay_ = SDL_CreateTexture(sdlRenderer_, SDL_PIXELFORMAT_IYUV,
                                 SDL_TEXTUREACCESS_STREAMING, width, height);

	setMaximumSize(QSize(width, height));

//	black_screen_ = SDL_CreateYUVOverlay(width, height, SDL_YV12_OVERLAY, screen_);
...
{
//	return new SdlVideoFrame(SDL_CreateYUVOverlay(screen_->w, screen_->h, SDL_YV12_OVERLAY, screen_));
    // canjian
//    return new SdlVideoFrame(SDL_CreateTexture(sdlRenderer_, SDL_PIXELFORMAT_IYUV, SDL_TEXTUREACCESS_STREAMING,
//                                               screen_->w, screen_->h));
}
...
bool SdlFfmpegAudioDecoder::decode(SdlAudioFrame* dst_frame, FfmpegFrame* src_frame)
{
    ...
    if (swr_ctx) {
        ...
        unsigned int out_size  = av_samples_get_buffer_size(NULL, spec.channels, dst_max_samples, spec.format, 0);
