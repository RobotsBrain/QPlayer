...
public:
    ...
    void write(T* ptr, size_t elems)
	{ 
	}

    void read(T* ptr, size_t elems) const
	{
	}

    bool has(size_t elems) const
	{
        if (end_ >= start_) {
            return end_ - start_ >= elems;
        } else {
            return (last_ - buf_) - (start_ - end_) >= elems;
        }
	}
